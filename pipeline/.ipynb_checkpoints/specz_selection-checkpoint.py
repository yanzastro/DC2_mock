...
#   functions that are most relevant for KiDS, etc. Due to differences between#
...
class dc2_data(object):
    """
    Base class providing easy access to magnitude columns of dc2 data tables.
    Filters can be accessed as class methods by there name, e.g.:
    dc2_data.i(suffix) would return the internally defined i-band magnitude
    ...
    def __init__(self, dc2table):
        assert(isinstance(dc2table, Table))
        self.data = dc2table
    ...
    def _get_filter_data(self, filter, suffix):
        ...
        if suffix is "":
            full_key = "%s" % (self.filter_keys[filter])
        else:
...
class KV450_dc2_data(dc2_data):
    """
    Sub-class of dc2_data defining the KiDS-VIKING specific mock catalogue
    magnitude columns and the detection band. Only six bands are available.
    Parameters
    ----------
    dc2table : astropy.table.Table
        dc2 data table.
...
class LSST_dc2_data(dc2_data):
    """
    Sub-class of dc2_data defining the LSST specific mock catalogue
...
class make_specz(object):
    ...
    ----------
    dc2data : dc2_data
        dc2 data table on which the selection function is applied.
    """
    ...
    def __init__(self, dc2data):
        assert(isinstance(dc2data, dc2_data))
        self.data = dc2data
        self.mask = np.ones(len(self.data), dtype="bool")
        self.redshift = dc2data["redshift"]
    ...
    def downsampling_N_tot(self, N_tot):
        """
        Method to randomly sample down the remaining dc2 objects to a given
...
class make_2dFLenS(make_specz):
    ...
    def __init__(self, dc2data):
        super().__init__(dc2data)
        # magnitudes
        self.g = dc2data.g("obs")
        self.r = dc2data.r("obs")
        self.i = dc2data.i("obs")
        self.z = dc2data.z("obs")
        self.k = dc2data.ks("obs")  # not available yet for Dc2
...
class make_SDSS_QSO(make_specz):
    ...
    def environmentCut(self):
        """
        Method create a fake dc2 quasar sample. Quasars do not exists in
        dc2, therefore the assumption is made that quasars sit in the central
...
class make_SDSS(make_specz):
    ...
    SDSS BOSS and SDSS QSO sample selections on an input dc2 mock catalogue.
    Parameters
    ----------
    dc2data : dc2_data
        dc2 data table on which the selection function is applied.
...
class make_DEEP2(make_specz):
    ...
    def __init__(self, dc2data):
        super().__init__(dc2data)
        # magnitudes
        self.B = self.data.g("")  # no B band available for cosmoDC2, use g band \
                                  # instead
        self.R = self.data.r("")
        self.I = self.data.i("")
...
class make_idealized(make_specz):
    ...
    def __call__(self, pass_survey_selection=False):
        stats = [self.input()]
        stats.append(self.photometryCut())
        fraction_KiDS = 0.575  # fraction of dc2 galaxies after KiDS selection
